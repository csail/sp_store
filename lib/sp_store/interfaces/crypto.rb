...
# Executable specification for the crypto algorithms used for the S-P store.
module Crypto
  # Creates an HMAC key.
  def self.hmac_key
  ...
  def self.sk_key
    ossl_sk_encryption.random_key
  end
  
  # Encrypts some data with a symmetric key.
  #
  # Args:
  #   key:: a symmetric key, generated by calling sk_key
  #   data:: string of data to be encrypted
  #
  # Returns an encrypted string. The encrypted string will be larger than the
  # original string.
  def self.sk_encrypt(key, data)
    cipher = ossl_sk_encryption.encrypt
    cipher.key = key
    cipher.padding = 0
  ...
  # Args:
  #   key:: a symmetric key, generated by calling sk_key
  #   encrypted_data:: an encrypted string obtained by calling sk_encrypt
  #
  # Returns the original string passed to sk_encrypt.
  def self.sk_decrypt(key, encrypted_data)
    cipher = ossl_sk_encryption.decrypt
    cipher.key = key
    cipher.padding = 0
    padded_data = cipher.update encrypted_data
    padding_bytes = padded_data[0, 1].unpack('C').first
    padded_data[1..-(padding_bytes + 1)]
  end
  ...
  # The OpenSSL symmetric encryption engine.
